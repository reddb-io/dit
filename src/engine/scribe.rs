...
//!
//! Implements [`Transcriber::stream`]: captures microphone audio, streams it
//! over a WebSocket to the Scribe API, and commits each stable segment via
//! the injector.  All session semantics from the original pipeline are
//! preserved:
//!
//!   * `partial_transcript`   → ignored (preview only).
//!   * `committed_transcript` → typed immediately; content-deduped.
//!   * On stop, an empty `commit: true` frame flushes the last segment, then
//!     we keep reading for `FINAL_WAIT_SECS` to catch trailing commits.
...
/// ElevenLabs Scribe v2 Realtime streaming transcription engine.
...
impl Transcriber for ScribeTranscriber {
    async fn stream(
        ...
        stream_session(cfg, injector, stop, state_tx).await
    }
}
