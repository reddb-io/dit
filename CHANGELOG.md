...
- add --features local to all release build targets
- add cargo build --features local step on Linux
- add Linux GUI system deps and --features gui build step


### Chores

- update Cargo.lock after rebase onto main (local engine deps)
- update Cargo.lock for local feature candle deps
...
- **language**: add --language auto support for local Whisper engine
- **doctor**: report local engine readiness (model present)
- **transcribe**: select engine based on cfg.engine at session start
- **engine**: export LocalEngine behind cfg(feature = "local")
- **engine**: add LocalEngine for offline Whisper inference
- **models**: add GGUF catalog entries and resolve_local_model
- **config**: add Engine enum and --engine local flag
- **build**: add local feature gating candle deps
- **main**: wire hold mode into manager and non-Linux hotkey path
- **linux_input**: react to key-release in hold mode
- **config**: add RecordingMode enum and --mode hold|toggle flag
